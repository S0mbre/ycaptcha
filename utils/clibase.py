...
class CLIBase:
    ...
    def __init__(self):
        self.commands = {f[4]: getattr(self, f) for f in dir(self) if callable(getattr(self, f)) and f.startswith('cmd_')}        
        ...
        self.usage2 = COLOR_HELP + '\t' + '\n\t'.join([(COLOR_STRESS + fn + ':\n' + COLOR_HELP + self.commands[fn].__doc__) for fn in self.commands if fn != QUIT_CMD])
